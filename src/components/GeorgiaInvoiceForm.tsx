...
function normalizeItems(invoice: GeorgiaInvoice | null): GeorgiaServiceItem[] {
  ...
  if (invoice?.service_description) {
      return [{ description: invoice.service_description, description_en: '', quantity: 1, unit_price: Number(invoice.amount) || 0, service_period: invoice.service_period ?? '' }];
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div><div className="flex items-center justify-between mb-2"><h3 className="text-sm font-semibold text-gray-800">Itens do serviço</h3><button type="button" onClick={addItem} className="text-sm text-blue-600 hover:text-blue-800">+ Adicionar item</button></div><div className="space-y-3">{items.map((item, index) => <div key={index} className="rounded-md border border-gray-200 p-3"><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><div><label className="block text-xs font-medium text-gray-600 mb-1">Descrição do Serviço</label><textarea value={item.description} onChange={e => updateItem(index, { description: e.target.value })} required rows={2} className="w-full border border-gray-300 rounded-md px-3 py-2" /></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Descrição em Inglês</label><textarea value={item.description_en ?? ''} onChange={e => updateItem(index, { description_en: e.target.value })} rows={2} placeholder="Opcional — usada no Download EN" className="w-full border border-gray-300 rounded-md px-3 py-2" /></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Quantidade</label><input type="number" min="1" step="1" value={item.quantity} onChange={e => updateItem(index, { quantity: parseFloat(e.target.value) || 0 })} className="w-full border border-gray-300 rounded-md px-3 py-2" /></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Preço unitário</label><input type="number" min="0" step="0.01" value={item.unit_price} onChange={e => updateItem(index, { unit_price: parseFloat(e.target.value) || 0 })} className="w-full border border-gray-300 rounded-md px-3 py-2" /></div><div className="md:col-span-2"><label className="block text-xs font-medium text-gray-600 mb-1">Período do serviço</label><input value={item.service_period} onChange={e => updateItem(index, { service_period: e.target.value })} placeholder="Ex.: 01/09/2026 – 30/09/2026" className="w-full border border-gray-300 rounded-md px-3 py-2" /></div></div>{items.length > 1 && <button type="button" onClick={() => removeItem(index)} className="mt-2 text-xs text-red-600 hover:text-red-800">Remover item</button>}</div>)}</div></div>
