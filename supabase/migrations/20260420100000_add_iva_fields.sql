-- Adds IVA (Portuguese VAT) management to clients, invoices, and
-- subscriptions. Each client opts-in to IVA via has_iva and stores its
-- own iva_percentage; invoices and subscriptions snapshot those values
-- at creation so historical documents stay stable even if the client
-- later changes their default.
--
-- Defaults preserve prior behaviour: the previous PDF generator always
-- assumed 23% IVA, so existing rows get has_iva=true and 23.00.

alter table public.clients
  add column if not exists has_iva boolean not null default true,
  add column if not exists iva_percentage numeric(5,2) not null default 23.00;
...
-- Replace the daily generator so freshly issued invoices snapshot the
-- subscription's IVA (which itself was snapshotted from the client at
-- creation time). Without this update, generated invoices would always
-- carry the column DEFAULT (23%) regardless of subscription overrides.
...
begin
  ...
  loop
    ...
    insert into public.invoices (number, client_id, subscription_id, status, issue_date, due_date, notes, has_iva, iva_percentage)
    values (invoice_number, sub.client_id, sub.id, 'pending', today, due_date,
            'Fatura gerada automaticamente da subscrição: ' || sub.name,
            sub.has_iva, sub.iva_percentage)
